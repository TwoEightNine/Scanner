...
import global.msnthrp.scanner.utils.setVisible


abstract class BaseAdapter<T, VH : RecyclerView.ViewHolder>(context: Context) : RecyclerView.Adapter<VH>() {
    ...
    fun updateNoNotify(items: List<T>) {
        this.items.clear()
        this.items.addAll(items)
        invalidateEmptiness()
    }
